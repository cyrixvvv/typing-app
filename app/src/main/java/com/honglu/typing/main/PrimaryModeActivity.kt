...
class PrimaryModeActivity : AppCompatActivity() {
    ...
    private fun updateDisplayText() {
        ...
        if (fullText.isEmpty()) {
            binding.tvDisplayText.text = getString(R.string.primary_waiting)
            return
        }

        val ssb = SpannableStringBuilder(fullText)

        // Already-typed characters: dim
        if (index > 0) {
            ssb.setSpan(
        ...
        // Current target character: bright highlight
